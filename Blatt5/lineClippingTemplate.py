...
def calcNewLine(Line, lineC, clipRegion):
    ...
    p = Line[0]
    q = Line[1]
    dx = q.coords[0]-p.coords[0]
    dy = q.coords[1]-p.coords[1]
    while p.reCode|q.reCode != 0:
        qc = q.coords
        pc = p.coords
        bits = 1<<3
        while True:
        ...
        if c == 8:
            ...
            changedPoint = max([p,q], key= lambda x:x.reCode)
            x = clipRegion[1][0]
            y = pc[1] + (qc[1]-pc[1])*(clipRegion[1][0]-pc[0])/(qc[0]-pc[0])
            if changedPoint == p:
                p = Point([x,y],clipRegion)
            else:
                q = Point([x,y],clipRegion)
            ...
            changedPoint = max([p,q], key= lambda x:x.reCode)
            x = clipRegion[0][0]
            y = pc[1] + (qc[1]-pc[1])*(clipRegion[0][0]-pc[0])/(qc[0]-pc[0])
            if changedPoint == p:
                p = Point([x,y],clipRegion)
            else:
                q = Point([x,y],clipRegion)
        lineC = p.reCode|q.reCode
