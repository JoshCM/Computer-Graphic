...
def intersect(l1, l2):
    ...
    def pointOnLine(sp,pp):
        x = sorted(pp,key = lambda x:x[0])
        y = sorted(pp,key = lambda x:x[1])
        return x[0][0]<= sp[0] <= x[1][0] and y[0][1]<= sp[1] <= y[1][1]
    polypoints = [array([x[0],x[1],1]) for x in l1]
    testpoints = [array([x[0],x[1],1]) for x in l2]

    polyline = cross(polypoints[0],polypoints[1])
    testline = cross(testpoints[0],testpoints[1])

    sp = cross(polyline,testline)
    sp = sp/sp[2]

    if sp[0] > testpoints[0][0] and pointOnLine(sp,polypoints):
        return True
    return False
