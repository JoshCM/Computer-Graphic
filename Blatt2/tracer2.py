...
p = Plane(Point(0,-1,0),Vector(0,2,0),(100,100,100),TEXTURE)
...
def shade(level, hitpointData):
	...
	return directColor + reflectedColor*REFLECTIONFACTOR
...
def computeDirectLight(hitpointData):
	...
	if s:
		return	((object.colorAt(hitpointData[1])-z) * 0.5 + LIGHTCOLOR *0.5*l.dot(n) + LIGHTCOLOR *0.2*(r.dot(ray.direction.negated()))**32)*SHADOWFACTOR
	else:
		return	(object.colorAt(hitpointData[1])-z) * 0.5 + LIGHTCOLOR *0.5*l.dot(n) + LIGHTCOLOR *0.2*(r.dot(ray.direction.negated()))**32
